...
# Load datasets
cause_meta <- read_csv("docs/death_causes.csv")
death_data <- read_csv("docs/unicef_indicator_1.csv")

# Step 1: Filter for "Top causes of death" in death_causes.csv
top_causes <- cause_meta %>%
  filter(Indicator == "Top causes of death") %>%
  select(Country, Level)  # Level contains the actual cause of death

# Step 2: Join with mortality data from unicef_indicator_1.csv
cause_with_deaths <- death_data %>%
  inner_join(top_causes, by = c("country" = "Country")) %>%
  group_by(Level) %>%
  summarise(DeathCount = sum(obs_value, na.rm = TRUE), .groups = "drop") %>%
  arrange(desc(DeathCount)) %>%
  slice_head(n = 5)

# Step 3: Create the bar chart
ggplot(cause_with_deaths, aes(x = reorder(Level, DeathCount), y = DeathCount)) +
  geom_bar(stat = "identity", fill = "tomato") +
  coord_flip() +
  labs(
    title = "The Silent Killers: Top 5 Causes of Adolescent Deaths",
    x = "Cause of Death",
    y = "Number of Deaths"
  ) +
  scale_y_continuous(labels = comma) +
